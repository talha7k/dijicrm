<script lang="ts">
  ...
  let { clientId, clientEmail, clientName, open = $bindable(false), onSendComplete }: Props = $props();
    ...
    let generatedDocuments = $state<Array<{ id: string; name: string; content: string; format: 'pdf' | 'html' }>>([]);
    let previewDocument = $state<{ id: string; name: string; content: string } | null>(null);
  ...
  async function handleSend() {
    ...
    try {
      ...
      const totalDocuments = generatedDocuments.length + selectedCustomDocs.length;
