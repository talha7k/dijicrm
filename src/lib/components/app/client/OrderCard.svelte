...
<div class="border rounded-lg overflow-hidden transition-all duration-200 hover:shadow-md bg-card">
    ...
    <div class="flex items-start justify-between">
      <div class="flex items-start space-x-3 flex-1 min-w-0">
        <div class="flex-shrink-0 mt-1">
          <svg class="h-5 w-5 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getOrderStatusIcon(order.status)}/>
          </svg>
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex items-center space-x-2">
            <h4 class="font-medium text-sm truncate text-foreground">{order.title}</h4>
          ...
          <p class="text-xs text-muted-foreground mt-1">
            Order #{order.id.slice(-6)} • {formatDate(order.createdAt?.toDate() || new Date())}
          </p>
          {#if order.description && !isExpanded}
      ...
      <div class="flex items-center space-x-2 flex-shrink-0">
        <div class="text-right">
          <p class="text-sm font-medium text-foreground">{formatCurrency(order.totalAmount)}</p>
  ...
  {#if isExpanded}
    <div class="border-t bg-background/50 backdrop-blur-sm">
      <!-- Invoice Header -->
      <div class="p-6 border-b bg-gradient-to-r from-muted/20 to-muted/10">
      ...
      <div class="p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[60%_40%] gap-6">
          ...
          <div class="space-y-6">
                ...
                <table class="w-full">
                  <thead class="bg-muted/30">
                    <tr class="border-b">
                      ...
                      <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider">Qty</th>
                      <th class="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">Unit Price</th>
                      <th class="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">Total</th>
                    </tr>
                  </thead>
            ...
            <!-- Order Metadata -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm bg-muted/20 p-4 rounded-lg">
              <div>
